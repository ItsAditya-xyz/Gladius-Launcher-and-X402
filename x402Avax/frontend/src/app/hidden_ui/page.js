  ...
  const formatContent = (text) => {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("# ")) {
        ...
        elements.push(
          <h2 key={i} className="text-xl font-bold mt-5 mb-2.5 text-foreground">
            {line.slice(3)}
          </h2>
        );
      } else if (line.startsWith("### ")) {
        ...
        elements.push(
          <pre key={i} className="bg-muted/50 text-foreground border border-border rounded-lg p-4 my-3 overflow-x-auto">
        ...
        <div className="flex justify-start mb-6">
          <div className="flex gap-3 max-w-3xl">
            <div className="flex-shrink-0 w-10 h-10 rounded-full">
              <img src={arenaImage.src} className="w-10 h-10 rounded-full" />
            </div>
    ...
    return (
      <div className={`flex ${isAssistant ? "justify-start" : "justify-end"} mb-6`}>
        <div className={`flex gap-3 max-w-3xl ${isAssistant ? "flex-row" : "flex-row-reverse"}`}>
          <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${isAssistant ? "" : "bg-secondary"}`}>
            {isAssistant ? (
              <img src={arenaImage.src} className="w-10 h-10 rounded-full" />
            ) : (
          ...
          <div className={`flex-1 px-5 py-3.5 rounded-2xl text-[15px] leading-relaxed ${isAssistantStyle}`}>
            {content ? (
              ...
              <span className="text-muted-foreground">Looking into the arena with my sword…</span>
            ) : (
              ""
            )}
          </div>
    ...
    <div className="h-screen flex flex-col bg-background">
          ...
          <div className="max-w-5xl mx-auto px-6 py-8">
            ...
            {errorMsg && (
              <div className="flex justify-center mb-6">
                <div className="max-w-3xl w-full bg-destructive/10 border border-destructive/40 rounded-xl px-5 py-3.5 text-sm text-destructive">
      ...
      <div className="flex-shrink-0 border-t border-border bg-card/80 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="bg-card rounded-2xl border border-border shadow-lg focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/20 transition-all">
            <textarea
              className="w-full resize-none outline-none px-5 pt-4 pb-2 text-[15px] text-foreground placeholder:text-muted-foreground bg-transparent"
              rows={2}
            ...
            <div className="flex items-center justify-between px-4 pb-3 pt-1">
              <div className="flex items-center gap-2">
                {loading ? (
                  <button
                    type="button"
                    onClick={stop}
                    className="flex items-center gap-2 px-4 py-2 bg-destructive hover:bg-destructive/90 text-destructive-foreground text-sm font-medium rounded-lg transition-colors"
                  >
                  ...
                  <>
                    <button
                      type="button"
                      onClick={onSubmit}
                      className="flex items-center gap-2 px-5 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-primary/20"
                      disabled={!input.trim()}
                    >
                    ...
                    <button
                      type="button"
                      onClick={regenerate}
                      className="flex items-center gap-2 px-4 py-2 text-muted-foreground hover:text-foreground hover:bg-muted/40 text-sm font-medium rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              ...
              <div className="text-xs text-muted-foreground">
                <kbd className="px-1.5 py-0.5 bg-muted/40 border border-border rounded text-xs font-mono">Enter</kbd>{" "}
                to send
              </div>
            </div>
          </div>

...
function getAppOrigin(req) {
  const fromEnv = (process.env.APP_ORIGIN || "").trim().replace(/\/+$/, "");
...
export async function GET(req, ctx) {
  ...
  const paymentInfo = (sessionHex) => ({
    // Agent UX: pay, then retry this same endpoint with X-402-Session
    next: {
      action: "pay_then_retry",
      retry: {
        method: "GET",
        url: origin ? `${origin}/api/${api.id}` : `/api/${api.id}`,
        headers: {
          "X-402-Session": sessionHex,
        },
      },
      poll: {
        until: "HTTP 200",
    ...
    agent_docs: origin ? `${origin}/agents.md` : "/agents.md",
    calls: {
      native: {
  ...
  try {
    ...
    if (api.onlyonce) {
      ...
      // Store mint result if the column exists on your schema
      try {
        ...
        if (mintErr) {
          console.error("failed to store mint_result", mintErr);
        } else {
          api.mint_result = mintResult;
        }
      } catch (err) {
        console.error("failed to store mint_result", err);
      }
    }
    ...
    return ok({
      ...
      data: {
        ...
        upstream: mintResult.upstream,
        mint_result: api.onlyonce ? mintResult : undefined,
      },
    });
  } catch (err) {

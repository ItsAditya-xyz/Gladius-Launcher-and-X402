...
function JSONPretty({ data }) {
  ...
  return (
    <pre className="bg-muted/50 text-foreground border border-border rounded-xl p-4 overflow-x-auto text-sm">
      {json}
    </pre>
  );
}
...
const id = Array.isArray(params?.id) ? params.id[0] : params?.id;
  ...
  const [consumedInfo, setConsumedInfo] = useState(null);
...
const doFetch = async (sessionOverride = null, opts = { fromPoll: false }) => {
  ...
  try {
    ...
    // One-shot consumed
    if (res.status === 410 && json?.code === "only_once_consumed") {
      if (allowDowngrade) {
        setView("consumed");
        setConsumedInfo(json?.data?.minted || null);
        setErrMsg("");
        setPayInfo(null);
      }
      return;
  ...
  const PayPanel = () => {
    ...
    return (
      <div className="w-full max-w-xl mx-auto bg-card rounded-2xl border border-border shadow-sm p-6">
        <h1 className="text-xl font-semibold text-foreground mb-1">Payment required</h1>
        <p className="text-muted-foreground text-sm mb-4">
          This endpoint is gated by <span className="font-mono">x402</span>. Complete the payment to unlock.
        </p>

        <div className="bg-muted/30 border border-border rounded-xl p-4 text-sm mb-4 break-all">
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Amount</span>
         <span className="font-medium text-foreground">{prettyAmount} ${tokenName}</span>
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <code className="text-[12px] text-muted-foreground">{payInfo.merchant_wallet}</code>
          </div>
          <div className="flex items-center justify-between mt-2">
            <span className="text-muted-foreground">Contract</span>
            <code className="text-[12px] text-muted-foreground">{payInfo.contract}</code>
          </div>
        </div>
  ...
  >
    {isConnected
      ...
      : "Connect wallet to pay"}
        ...
        {errMsg ? (
          <div className="mt-4 text-sm text-destructive bg-destructive/10 border border-destructive/40 rounded-lg p-2">
            {errMsg}
          </div>
        ) : null}
  ...
  const ConsumedPanel = () => {
    return (
      <div className="w-full max-w-3xl mx-auto">
        <div className="bg-card rounded-2xl border border-border shadow-sm p-4 mb-4">
          <h2 className="text-lg font-semibold text-foreground">Already minted</h2>
          <p className="text-muted-foreground text-sm">
            This launch endpoint has already been triggered once.
          </p>
        </div>
        ...
        {consumedInfo ? (
          <div className="bg-card rounded-2xl border border-border shadow-sm p-4">
            <JSONPretty data={consumedInfo} />
          </div>
        ) : (
          <div className="bg-card rounded-2xl border border-border shadow-sm p-4 text-sm text-muted-foreground">
            Mint details are not available for this endpoint.
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {view === "paying" && (
          <div className="w-full max-w-xl mx-auto bg-card rounded-2xl border border-border shadow-sm p-6 text-center">
            <div className="text-foreground font-semibold mb-2">Waiting for confirmation…</div>
            <p className="text-muted-foreground text-sm">
        ...
        {view === "consumed" && <ConsumedPanel />}

        {view === "error" && (
          <div className="w-full max-w-xl mx-auto bg-card rounded-2xl border border-destructive/40 shadow-sm p-6">
        ...
        <div className="mt-8 text-center text-sm text-muted-foreground">
          New to x402? Read the <a href="/docs" className="text-primary hover:underline">docs</a>.
        </div>
      </main>
    </div>
  );

  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <section className="mb-10">
          <h3 className="text-lg font-semibold text-foreground mb-2">Create a Masked URL</h3>
          <p className="text-muted-foreground mb-3">Two options:</p>
          <ul className="list-disc pl-6 text-muted-foreground space-y-1 mb-4">
            <li>Use the UI at <Link href="/gate" className="text-primary hover:underline">/gate</Link>.</li>
            <li>Call the API: <span className="font-mono">POST /api/402/apis</span> with JSON body:</li>
          </ul>
          <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`POST /api/402/apis
...
{ "apiId": "<uuid-or-id>" }`}</code></pre>
        ...
        <section className="mb-10">
          <h3 className="text-lg font-semibold text-foreground mb-2">Calling the Masked API (Agent Flow)</h3>
          <ol className="list-decimal pl-6 text-muted-foreground space-y-3">
            <li>
              Request the masked endpoint without a session header:
              <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`curl -i ${'${ORIGIN}'}/api/<apiId>`}</code></pre>
              Expected response: <span className="font-mono">HTTP 402</span> with JSON payment instructions.
              <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`Status: 402
{
    ...
    "calls": {
      ...
      "erc20_permit": { /* optional permit flow descriptor */ }
            ...
            <li>
              ...
              <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`curl -s ${'${ORIGIN}'}/api/<apiId> \
  -H "X-402-Session: 0x<session_id>" -i`}</code></pre>
              ...
              <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`Status: 200
{
  "code": "successful",
  "data": {
    ...
    "upstream": {
      ...
      "body": <json|string|{ image_url: string }>
        ...
        <section className="mb-10">
          <h3 className="text-lg font-semibold text-foreground mb-2">Agent Integration Example (Node + viem/ethers pseudo)</h3>
          <pre className="bg-muted/50 text-foreground border border-border p-4 rounded-xl overflow-x-auto text-sm"><code>{`import fetch from 'node-fetch';
...
async function callMasked(apiId) {
  ...
  throw new Error('Timeout waiting for unlock');
        ...
        <section className="mb-10">
          <h3 className="text-lg font-semibold text-foreground mb-2">Human UI</h3>
          <p className="text-muted-foreground">
        ...
        <section className="mb-10">
          <h3 className="text-lg font-semibold text-foreground mb-2">Response Shapes</h3>
          <ul className="list-disc pl-6 text-muted-foreground space-y-2">
            <li>402 payment required: <span className="font-mono">{`{ code: "payment_required", data: { session_id, network, contract, amount_wei, merchant_wallet, token_address, agent_docs, calls, next } }`}</span></li>
        ...
        <div className="text-xs text-muted-foreground">Contract: 0xDa90Fac43937AD84dC9483ff118C8c2CEc5f1F56 · Network: Avalanche C-Chain</div>
      </main>
    </div>
  );
}
